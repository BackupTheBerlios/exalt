...
void
_config_exalt_module(void *data, Config_Item *ci)
{
   E_Config_Dialog      *cfd;
   ...
   snprintf(buf, sizeof(buf), "%s/module.edj",
	     e_module_dir_get(exalt_config->module));

   con = e_container_current_get(e_manager_current_get());
   cfd = e_config_dialog_new(con, D_("Exalt module Configuration"), "Exalt", 
			     "_exalt_config_dialog", buf, 0, v, ci);
